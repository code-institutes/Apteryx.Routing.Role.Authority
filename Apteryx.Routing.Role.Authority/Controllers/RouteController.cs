...
public class RouteController : ControllerBase
{
    ...
    public async Task<IActionResult> Post([FromBody] AddRouteModel model)
    {
        ...
        var action = await _db.ApteryxRoute.Commands.FindOneAsync(f => f.Path == path && f.Method == method);
        if (action != null)
        ...
        await _db.ApteryxRoute.Commands.InsertAsync(route);
        //记录日志
    ...
    public async Task<IActionResult> Put([FromBody] EditRouteModel model)
    {
        ...
        var route = await _db.ApteryxRoute.Commands.FindOneAsync(f => f.Id == routeId);
        if (route == null)
        ...
        var check = await _db.ApteryxRoute.Commands.FindOneAsync(f => f.Path == path && f.Method == method);
        if (check != null)
            if (check.Id != routeId)
        ...
        var result = await _db.ApteryxRoute.Commands.FindOneAndReplaceOneAsync(f => f.Id == route.Id, route);
        //记录日志
    ...
    {
        return Ok(ApteryxResultApi.Susuccessful(await _db.ApteryxRoute.Commands.FindOneAsync(f => f.Id == id)));
    }
    ...
    public async Task<IActionResult> Delete([SwaggerParameter("路由ID", Required = true)] string id)
    {
        ...
        var route = await _db.ApteryxRoute.Commands.FindOneAsync(f => f.Id == id);
        if (route == null)
            return Ok(ApteryxResultApi.Fail(ApteryxCodes.路由不存在));
        if (route.AddType != AddTypes.人工)
        ...
        //将路由从所有角色中解除关联
        await _db.ApteryxRole.Commands.UpdateManyAsync(u => u.RouteIds.Contains(id), Builders<Role>.Update.Pull(p => p.RouteIds, id));
        //删除路由
        var result = await _db.ApteryxRoute.Commands.FindOneAndDeleteAsync(d => d.Id == id);
        //记录日志
    ...
    {
        return Ok(ApteryxResultApi.Susuccessful(_db.ApteryxRoute.Commands.FindAll()));
    }
    ...
    public  ActionResult<ApteryxResult<List<Route>>> GetRefresh()
    {
        ...
        var item = _db.ApteryxRoute.Commands.FindAll().GroupBy(g => g.CtrlName).Select(s => new ResultGroupRouteModel()
        {
            CtrlName = s.Key,
