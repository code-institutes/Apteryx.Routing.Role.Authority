    ...
    public sealed class ConsoleAuthorizeAttribute : Attribute, IAuthorizationFilter, IActionFilter
    {
        ...
        public void OnActionExecuted(ActionExecutedContext context)
        {
            ...
            try
            {
                ...
                _db.ApteryxCallLog.Immediate.UpdateOne(u => u.TraceIdentifier == context.HttpContext.TraceIdentifier, Builders<CallLog>.Update.Set(s => s.Response, response));
                return;
            }
            catch { }
            return;
        ...
        public void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (httpContext.User.Identity != null && !httpContext.User.Identity.Name.IsNullOrWhiteSpace())
            {
                ...
                callLog.SystemAccount = _db.ApteryxSystemAccount.Immediate.FindOne(accountId);
            }
            ...
            if (!context.ModelState.IsValid)
            {
                ...
                _db.ApteryxCallLog.Immediate.Insert(callLog);
                return;
            }
            _db.ApteryxCallLog.Immediate.Insert(callLog);
            return;
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            if (context.HttpContext.User.Identity != null && context.HttpContext.User.Identity.IsAuthenticated)
            {
                ...
                var systemAccount = _db.ApteryxSystemAccount.Immediate.FindOne(f => f.Id == accountId);
                ...
                var route = _db.ApteryxRoute.Immediate.FindOne(f => f.Method == method && f.Path == template);
                if (route != null)
                {
                    var roleRoute = _db.ApteryxRole.Immediate.FindOne(f => f.Id == systemAccount.RoleId && f.RouteIds.Contains(route.Id));
                    if (roleRoute != null)
                    {
                        return;
                    }
                    ...
                    {
                        var role = _db.ApteryxRole.Immediate.FindOne(systemAccount.RoleId);
